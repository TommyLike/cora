...
import (
	...
	"github.com/cncf/community-cli/internal/builder"
	"github.com/cncf/community-cli/internal/config"
	"github.com/cncf/community-cli/internal/executor"
	"github.com/cncf/community-cli/internal/registry"
...
func run() error {
	...
	root := &cobra.Command{
		Use:   "community",
		Short: "community-cli — unified access to open-source community services",
		Long: `community-cli aggregates access to community services
...
driven by OpenAPI specs published by each backend service.`,
	...
	if err := injectServiceCommands(root, reg, cfg, exec); err != nil {
		// Non-fatal: print a warning so "community --help" still works.
...
func injectServiceCommands(
	...
	if svcName == "" {
		// No service specified (e.g. "community --help"); register all services
...
// buildSpecCmd returns `community spec <subcommand>` for cache management.
